...
// K
const RATING_COEFFICIENT_CEILING = 50
...
const ratingCoefficient = (previousGamesPlayed) => {
  ...
  if (previousGamesPlayed < QUALIFIER_MATCHES) {
    return (floorCeilingDiff - ((floorCeilingDiff/QUALIFIER_MATCHES) * previousGamesPlayed)) + RATING_COEFFICIENT_FLOOR
  }
...
const calculateWinExpectancy = (playerTeamRating, opponentTeamRating, weightFactor) => {
  ...
  return 1/((10 * (-ratingDifference/weightFactor)) + 1)
}

const calculateNewRating = ({ playerRating, teammateRating, opponentDefRating, opponentOffRating, teamPoints, previousGamesPlayed }) => {
...
class GamesController extends Controller {
  ...
  static async updateElos(req, res, next) {
    ...
    let playersToUpdate = uniqBy([
      { playerId: red_def, previousGamesPlayed: red_def_games_played, playerRating: red_def_rating, teammateRating: red_off_rating, opponentDefRating: black_def_rating, opponentOffRating: black_off_rating, teamPoints: red_points},
      { playerId: red_off, previousGamesPlayed: red_off_games_played, playerRating: red_off_rating, teammateRating: red_def_rating, opponentDefRating: black_def_rating, opponentOffRating: black_off_rating, teamPoints: red_points},
      { playerId: black_def, previousGamesPlayed: black_def_games_played, playerRating: black_def_rating, teammateRating: black_off_rating, opponentDefRating: red_def_rating, opponentOffRating: red_off_rating, teamPoints: black_points},
      { playerId: black_off, previousGamesPlayed: black_off_games_played, playerRating: black_off_rating, teammateRating: black_def_rating, opponentDefRating: red_def_rating, opponentOffRating: red_off_rating, teamPoints: black_points}
    ], 'playerId')
